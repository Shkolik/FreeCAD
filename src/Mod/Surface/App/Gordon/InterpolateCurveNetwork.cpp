    ...
    , m_spatialTol(spatialTol)
{
    ...
    if (uniqueProfiles.size() < 2) {
        throw error("There must be at least two unique profiles for the curve network interpolation.",
                    MATH_ERROR);
    }
    ...
    // Copy the curves
    for (std::vector<Handle(Geom_BSplineCurve)>::const_iterator it = uniqueProfiles.begin();
         it != uniqueProfiles.end(); ++it) {
        m_profiles.push_back(*it);
    }
    for (std::vector<Handle(Geom_BSplineCurve)>::const_iterator it = uniqueGuides.begin();
         it != uniqueGuides.end(); ++it) {
        m_guides.push_back(* it);
    }
}
...
{    
    ...
    /*warning("\nMakeCurvesCompatible() Tolerance: " << std::to_string(m_spatialTol) << "\n");*/
    ...
    if (isClosedProfile) {        
        ...
        for (int spline_u_idx = 0; spline_u_idx < nProfiles; ++spline_u_idx) {
            ...
            intersection_params_u(spline_u_idx, nGuides - 1) =
                tmp_intersection_params_u(spline_u_idx, 0) < BSplineAlgorithms::PAR_CHECK_TOL
                ? 1.0
        ...
        for (int spline_v_idx = 0; spline_v_idx < nGuides; ++spline_v_idx) {
            ...
            intersection_params_v(nProfiles - 1, spline_v_idx) =
                tmp_intersection_params_v(0, spline_v_idx) < BSplineAlgorithms::PAR_CHECK_TOL
                ? 1.0 
    ...
    for (int spline_u_idx = 0; spline_u_idx < nProfiles; ++spline_u_idx) {
        ...
        //printCurve(profile, spline_u_idx, "reparametrizing u curve Profile");
        ...
        //printCurve(profile, spline_u_idx, "new u curve Profile");
    }
    ...
    for (int spline_v_idx = 0; spline_v_idx < nGuides; ++spline_v_idx) {
        ...
        //printCurve(guide, spline_v_idx, "reparametrizing v curve Guide");
        ...
        //printCurve(guide, spline_v_idx, "new v curve Guide");
    }
