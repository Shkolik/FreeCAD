                                                    ...
                                                    const std::vector<double>& intersection_params_spline_v)
{
    ...
    /* printVector(intersection_params_spline_u, "Params_U ");
    printVector(intersection_params_spline_v, "Params_V ");

    print2DArray(intersection_pnts, "intersection_pnts");*/
    ...
    /*warning("curve_u_tolerance: " + std::to_string(curve_u_tolerance) + " curve_v_tolerance: "
            + std::to_string(curve_v_tolerance) + " tp_tolerance: " + std::to_string(tp_tolerance));*/
    ...
    /*warning("-   Skinning profiles with:\n continuous = " + std::to_string(makeVClosed) + "\nintersection_params_spline_v="
            + getVectorStr(intersection_params_spline_v));*/
    ...
    warning("-   Skinning profiles with:\n continuous = " + std::to_string(makeUClosed)
            + "\nintersection_params_spline_u=" + getVectorStr(intersection_params_spline_u));*/
                                                               ...
                                                               double tol)
{
    ...
    if (std::abs(intersection_params_spline_u.front()) > splines_scale * tol || std::abs(intersection_params_spline_u.back() - 1.) > splines_scale * tol) {
        throw error(
            "WARNING: B-splines in u-direction mustn't stick out, spline network must be 'closed'!  intersection_params_spline_u.front:"
                    + std::to_string(intersection_params_spline_u.front())
                    + " intersection_params_spline_u.back:"
                    + std::to_string(intersection_params_spline_u.back())
                    + " splines_scale*tol:" + std::to_string(splines_scale * tol)
        );
    } 
